  ...
  public:
    ...
    void init() {
      ...
      Cut lepton_cut = ((Cuts::abseta < 2.4) and 
                        (Cuts::pT > 8.*GeV) and 
                        (((Cuts::abspid == PID::ELECTRON) and (Cuts::abseta < 2.5)) or (Cuts::abspid == PID::MUON)));
    ...
    void analyze(const Event& event) {
      ...
      if ((abs(dressedLeptons.at(0).pid() + dressedLeptons.at(1).pid() + dressedLeptons.at(2).pid()) != 11) or (abs(dressedLeptons.at(0).pid() + dressedLeptons.at(1).pid() + dressedLeptons.at(2).pid()) != 13))   vetoEvent;
      ...
      for (int iL = 0; iL < int(dressedLeptons.size()); iL++) {
        ...
        if (abs(dressedLeptons.at(iL).pid() + dressedLeptons.at(jL).pid()) == 0) {
          if ((dressedLeptons.at(iL).momentum() + dressedLeptons.at(jL).momentum()).mass() <= 4.*GeV) vetoEvent;
        }
        }
      }
      
      // Additional Kinematic Requirement: 
      	// All identified lepton pairs decaying from Z must have at least an invariant mass of 60 < mll' < 120.
      for (int iL = 0; iL < int(dressedLeptons.size()); iL++) {
        for (int jL = iL + 1; jL < int(dressedLeptons.size()); jL++) {
        if (abs(dressedLeptons.at(iL).pid() + dressedLeptons.at(jL).pid()) == 0) {
          if (((dressedLeptons.at(iL).momentum() + dressedLeptons.at(jL).momentum()).mass() <= 60.*GeV) or ((dressedLeptons.at(iL).momentum() + dressedLeptons.at(jL).momentum()).mass() >= 120.*GeV)) vetoEvent;
        }
        }
      }
